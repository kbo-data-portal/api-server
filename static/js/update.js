...
function updateTeamInfo(response) {
  ...
  teamHomeInfo.empty();
  teamHomeInfo.append(`
    <div class="name" style="color: ${response.home_team_color};">${response.home_team_name}</div>
  ...
  teamAwayInfo.append(`
    ...
    <div class="name" style="color: ${response.away_team_color};">${response.away_team_name}</div>
  `);
}
...
function updateMatchGraph(response) {
  createPie('graph-all-match-left', `${response.overall_vs_record[0] + response.overall_vs_record[1] + response.overall_vs_record[2]}<br/>GAME`,
    [
      { name: `${response.home_team_name} 승`, y: response.overall_vs_record[0], color: response.home_team_color },
      { name: '무', y: response.overall_vs_record[1], color: '#666666' },
      { name: `${response.away_team_name} 승`, y: response.overall_vs_record[2], color: response.away_team_color }
    ]
  );
  ...
  createPie('graph-match-left', `${response.season_vs_record[0] + response.season_vs_record[1] + response.season_vs_record[2]}<br/>GAME`,
    [
      { name: `${response.home_team_name} 승`, y: response.season_vs_record[0], color: response.home_team_color },
      { name: '무', y: response.season_vs_record[1], color: '#666666' },
      { name: `${response.away_team_name} 승`, y: response.season_vs_record[2], color: response.away_team_color }
    ]
  );
