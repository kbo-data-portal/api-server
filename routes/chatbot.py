...
from fetcher.game import fetch_game_prediction
...
def _get_error_template(output):
    return {
        "version": "2.0",
        "template": {"outputs": [{"simpleText": {"text": output}}]},
    }
...
def _get_stat_comparison(
    home_nm, away_nm, home_stat, away_stat, convert=False, reverse=False
):
    if convert:
...
def team_schedule():
    ...
    for schedule in game_schedule[:5]:
        team = (
            schedule.AWAY_NM if schedule.HOME_NM == request_team else schedule.HOME_NM
        )
    ...
    if not items:
        return jsonify(
            _get_error_template(f"죄송합니다. {request_team}의 예정된 경기가 없습니다.")
        )
    ...
    return jsonify(
        _get_template(
            {
                "listCard": {
                    "header": {"title": f"⚾ {TEAMS[request_team]["full"]} 경기 일정"},
                    "items": items,
                }
            }
        )
...
def game_detail():
    ...
    return jsonify(_get_template({"basicCard": card}))
